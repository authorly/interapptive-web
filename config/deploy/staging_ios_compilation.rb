...
namespace :god do
  ...
  task :restart, :roles => :staging_ios_compilation do
...
namespace :deploy do
  ...
  task :copy_god_restart_script, :roles => :staging_ios_compilation do 
  ...
  task :copy_gemfile_to_crucible, :roles => :staging_ios_compilation do
  ...
  task :copy_keychain_password, :roles => :staging_ios_compilation do
    run "cp #{shared_path}/config/keychain_password.txt #{release_path}/config"
  end

  desc "Copies ActionMailer configuration file to config"
  task :copy_action_mailer_options, :roles => :staging_ios_compilation do
